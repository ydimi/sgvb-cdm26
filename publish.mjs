...
const NAV_BAR = `<style>
  ...
  .sgvb-nav .in { display:flex; flex-wrap:wrap; gap:6px; justify-content:center; align-items:center; max-width:1000px; margin:0 auto; padding:8px 10px; }
  .sgvb-nav a { color:#e6edf3; text-decoration:none; padding:6px 10px; border-radius:8px; background:#161b22; border:1px solid #2a313c; white-space:nowrap; font:600 13px/1 system-ui,-apple-system,Segoe UI,Roboto,sans-serif; }
</style>
<nav class="sgvb-nav"><div class="in">${NAV_ITEMS.map(([href, label]) => `<a href="${href}">${label}</a>`).join("")}</div></nav>
<script>(function(){function p(){var n=document.querySelector('.sgvb-nav');if(n)document.body.style.paddingTop=(n.offsetHeight+6)+'px';}p();window.addEventListener('resize',p);})();</script>`;
